...
namespace Weave.RssAggregator.Core.Services.Feed_Parsing
{
    internal class RssParsingMediaParserHelper
    {
        internal string ImageUrl { get; set; }
        internal string YoutubeId { get; set; }
        internal string VideoUri { get; set; }
        internal string PodcastUri { get; set; }
        internal string ZuneAppId { get; set; }

        internal void ExtractYoutubeVideoAndPodcastUrlsFromDescription(string descriptionText)
        {
            ...
            var potentialImage = matches
                .Where(o => o.IsImageUrl())
                .FirstOrDefault();
            ...
            var potentialVideo = matches
                .Where(o =>
                    o.EndsWith(".mp4", StringComparison.OrdinalIgnoreCase))
                .FirstOrDefault();

            VideoUri = potentialVideo;


            var potentialPodcast = matches
                .Where(o => o.IsAudioFileUrl())
                .FirstOrDefault();

            PodcastUri = potentialPodcast;
        }
